...
#Retrieving youtube data using api key:

api_service_name = "youtube"
api_version = "v3"
api_key= "AIzaSyAD736C1J8WISbH0M4i5rYlV1iovFhCB7k"
youtube = googleapiclient.discovery.build(api_service_name, api_version, developerKey=api_key)

#Creating user interface using streamlit:

st.set_page_config(page_title="Youtube project", page_icon=":tada:", layout="wide")
st.image("D:\TKS\youtube.png", width=200, use_column_width=False)
st.markdown("<h3 style='text-align: left; color: red; font-size: 35px;'>Data Harvesting & Warehousing</h3>", unsafe_allow_html=True)
st.markdown("<h3 style='text-align: left; color: black; font-size: 20px;'>Enter YouTube Channel ID</h3>", unsafe_allow_html=True)
...
#Creating database and channel table in Mysql:
...
#Creating database and video table in Mysql:
    ...
    mycursor.execute('''create table if not exists video( channel_id VARCHAR(50),video_id VARCHAR(50) PRIMARY KEY ,
                                    ...
                                    duration VARCHAR(10),thumbnail VARCHAR(100),caption_status VARCHAR(50),tags VARCHAR(1000),
                                    FOREIGN KEY (channel_id) REFERENCES channel(channel_id))''')
    my_connection.commit()
    Video_details.to_sql('video',engine,if_exists='append',index=False)

#Creating database and comment table in Mysql:

    mycursor.execute('create database if not exists youtube')
    mycursor.execute('use youtube')
    mycursor.execute('''create table if not exists Comment( video_id VARCHAR(50), comment_id VARCHAR(50),
                                comment_text VARCHAR(10000) ,comment_author VARCHAR(1000),comment_published_at DATETIME,
    ...
    st.success('**Successfully uploaded in youtube Database**')

#To view uploaded channels and queries:

tab1,tab2 = st.tabs(["Uploaded channels", "Queries"])
